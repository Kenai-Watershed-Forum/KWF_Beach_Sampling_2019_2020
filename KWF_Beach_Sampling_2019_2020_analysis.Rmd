...
z <- dat %>%
  ...
  # spread indv exceedence classes in to columns
  ...
  # if >10%, assign exceedence class per regs
  
  # modify to create this all in one column rather than 3
  
  
  mutate(ec_excd_130_30day = case_when((ec_excd_130_30day_pct > 10) ~ "ec_excd_130"),
         fc_excd_31_30day = case_when((fc_excd_31_30day_pct > 10) ~ "fc_excd_31"),
         fc_excd_400_30day = case_when((fc_excd_400_30day_pct > 10) ~ "fc_excd_400")) %>%
  
  
  # woeking here to format for use in tile plot!
  
  
  
  select(location,year,data_type,date,gm30_excd,
         ec_excd_130_30day,
         fc_excd_31_30day,
         fc_excd_400_30day)
...
# working here to maybe adapt code from chunk 13 to prep for plot in chunk 12
...
#(tileplot 
  ...
  filter(!is.na(excd_val),
          excd_val %ni% c("no_gm_excd","no_indv_excd","")) %>%
  mutate(day = yday(date),
         date = ymd(date)) %>%
    
     # plot
    ggplot(aes(date,excd_val)) +
    geom_tile(aes(fill = excd_val)) +
    ...
    theme_bw() +
     theme(strip.text.y = element_text(angle = 0),
           axis.text.x = element_text(angle = 35, margin = margin(t=20)),
    ...
    # modify legend text
    scale_fill_discrete(name="Exceedence Type (CFU/100 mL)",
...
# save plot
ggsave("output/exceedence_by_date.png", width = 10, height = 11)

# shoot... does the indv samp assignment acct for 3 days prior data or just indv val?
...
Perform calculations to determine if 10% of individual sample values in prior rolling 30 day period have exceeded thresholds
Format as wide table for easier viewing in report

```{r}

z <- dat %>%
  filter(!is.na(val)) %>%
  select(location,year,date,data_type,val,gm30,units,qualifier,indv_excd,gm30_excd) %>%
  group_by(data_type,location,year) %>%
  mutate(obs = 1) %>%
...
# pivot
z1 <- z %>%
  pivot_longer(cols = contains("gm30_"), names_to = "test", values_to = "val")


# maybe
# prep wide format for view in report
z1 <- z %>%
  select(data_type,location,date,gm30)
...
# make table long and refdo tile figure
...
### reconsider all code belwo here base on corrected excd designation interpretation
