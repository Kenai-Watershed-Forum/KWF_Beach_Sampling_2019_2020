...
(tileplot <- dat %>%
  ...
  mutate(day = yday(date),
    ...
    scale_fill_manual(name="Exceedence Type (CFU/100 mL)",
                      ...
                      values=c("#FFFF00","#999999", "#E69F00", "#56B4E9","#CCCC33","#FFFFFF","#FFFFFF")) 
  )
...
Create table; calculate % exceedence frequency by site, year, and bacteria type for individual samples
...
# % of days exceeding criteria for gm30 values
gm30_pct <- dat %>%
  group_by(location,year,data_type) %>%
  count(gm30_excd) %>%
  mutate(pct = prop.table(n)) %>%
...
-- as time allows, do a simple run and/or model valiudation of VB model
...
```{r}
dat %>%
  spread(data_type,val)
  pivot_wider(id_cols = c("data_type","val"),names_from = "data_type",values_from = "val") %>%
  unlist() %>%
  data.frame()
  ggplot(aes())
